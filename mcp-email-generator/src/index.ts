...
import { validateEmailHtml, ValidationResult } from "./validators.js";
...
支持纯文本和HTML两种格式输出。`,
        ...
        },
        {
          ...
          description: `检测HTML邮件的质量，包括链接检测、文本检测、图片检测、HTML结构检测和垃圾邮件评分。
返回详细的检测报告，包括各项评分、发现的问题和优化建议。`,
          inputSchema: {
            type: "object",
            properties: {
              html: {
                type: "string",
                description: "需要检测的HTML邮件内容",
              },
              subject: {
                type: "string",
                description: "邮件主题（可选，用于主题行检测）",
              },
            },
            required: ["html"],
          },
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (name) {
        ...
        case "validate_email_html":
          return this.handleValidateEmailHtml(args);
        default:
  ...
  private handleGenerateEmail(args: unknown) {
    const { 
      topic, 
      points, 
      tone = "formal", 
  ...
  private handleGenerateReplyEmail(args: unknown) {
    const { 
      original_email, 
      reply_points = [], 
      reply_type, 
  ...
  private handlePolishEmail(args: unknown) {
    const { email_content, polish_type = "professional", language = "zh", format = "text" } = args as {
  ...
  private handleGenerateHtmlEmail(args: unknown) {
    const {
      title,
      headline,
      content,
    ...
    const htmlEmail = this.generateHtmlEmail(
      title, headline, content, cta_text, cta_link, 
      template, language, sender_name, sender_logo, primary_color
    );
  ...
  private handleValidateEmailHtml(args: unknown) {
    const { html, subject } = args as {
      html: string;
      subject?: string;
    };

    if (!html) {
      throw new McpError(ErrorCode.InvalidParams, "缺少必要参数：html");
    }

    const result = validateEmailHtml(html, subject);
  ...
  private generateEmailContent(
    ...
    if (safeFormat === "html" || safeFormat === "both") {
      result.html = this.generateTextToHtml(
        subject, 
        textContent, 
        sender, 
        safeLanguage,
  ...
  private generateTextContent(
    ...
    const langKey = safeLanguage as keyof typeof greetings;
  ...
  private renderHtmlTemplate(
    ...
    const templates: Record<string, (d: typeof data) => string> = {
  ...
  // 默认模板
  private defaultTemplate(d: typeof this.renderHtmlTemplate extends (template: string, data: infer D) => string ? D : never): string {
...
<body style="margin: 0; padding: 0; background-color: #f3f4f6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  ...
  // 新闻通讯模板
  private newsletterTemplate(d: typeof this.defaultTemplate extends (d: infer D) => string ? D : never): string {
...
<body style="margin: 0; padding: 0; background-color: #0f172a; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  ...
  // 公告模板
  private announcementTemplate(d: typeof this.defaultTemplate extends (d: infer D) => string ? D : never): string {
...
<body style="margin: 0; padding: 0; background-color: #fef3c7; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  ...
  // 邀请函模板
  private invitationTemplate(d: typeof this.defaultTemplate extends (d: infer D) => string ? D : never): string {
...
<body style="margin: 0; padding: 0; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  ...
  // 通知模板
  private notificationTemplate(d: typeof this.defaultTemplate extends (d: infer D) => string ? D : never): string {
...
<body style="margin: 0; padding: 0; background-color: #f3f4f6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  ...
  // 营销模板
  private marketingTemplate(d: typeof this.defaultTemplate extends (d: infer D) => string ? D : never): string {
...
<body style="margin: 0; padding: 0; background-color: #f3f4f6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
  ...
  private generateReplyContent(
    ...
    const intro = replyTypeMessages[replyType as keyof typeof replyTypeMessages] || replyTypeMessages.custom;
  ...
  // 润色邮件
  private polishEmailContent(
    emailContent: string,
    polishType: string,
    language: string,
    format: string
  ): { original: string; text?: string; html?: string; suggestions: string[] } {
    const suggestions = {
    ...
    const result: { original: string; text?: string; html?: string; suggestions: string[] } = {
      original: emailContent,
      suggestions: suggestions[polishType as keyof typeof suggestions] || suggestions.professional,
    };
