...
class EmailGeneratorHttpServer {
    ...
    start() {
        ...
        server.listen(this.port, () => {
            ...
            console.log(`   API:    POST /generate-email, /generate-reply, /polish-email, /generate-html-email`);
            if (this.apiKey) {
    ...
    async handleGenerateEmail(req, res) {
        ...
        const result = this.generateEmailContent(topic, points, tone || "formal", recipient, sender, language || "zh", format || "text", template || "default");
        this.sendJson(res, result);
    }
    async handleGenerateReply(req, res) {
        const body = await this.parseBody(req);
        ...
        const result = this.generateReplyContent(original_email, reply_points || [], reply_type, tone || "formal", format || "text");
        this.sendJson(res, result);
    }
    async handlePolishEmail(req, res) {
        const body = await this.parseBody(req);
        ...
        const result = this.polishEmailContent(email_content, polish_type || "professional", language || "zh", format || "text");
        this.sendJson(res, result);
    }
    async handleGenerateHtmlEmail(req, res) {
        const body = await this.parseBody(req);
        ...
        const result = this.generateHtmlEmail(title, headline, content, cta_text, cta_link, "default", language || "zh", sender_name, sender_logo, primary_color || "#2563eb");
        this.sendJson(res, result);
    }
    // ===== 核心业务逻辑 =====
    generateEmailContent(topic, points, tone, recipient, sender, language, format, template) {
        ...
        if (format === "html" || format === "both")
            result.html = this.generateTextToHtml(subject, textContent, sender, language, template || "default");
        return result;
    }
...
<body style="margin: 0; padding: 0; background-color: #f3f4f6; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif;">
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background-color: #f3f4f6;">
    ...
    generateReplyContent(originalEmail, replyPoints, replyType, tone, format) {
        ...
        const intro = messages[replyType] || messages.custom;
    ...
    polishEmailContent(emailContent, polishType, language, format) {
        ...
        const result = { original: emailContent, suggestions: suggestions[polishType] || suggestions.professional };
        if (format === "text" || format === "both")
