define("vs/tsMode-BWLDZXKb", ["exports", "./ts.worker-hBUOdPKT", "./editor-ECyZhXSU", "./workers-DFrw9KmQ", "./monaco.contribution-Bj7ZDQQk"], (function(exports, ts_worker, editor, workers, src_deprecated_language_typescript_monaco_contribution_ts) {
  "use strict";
  class WorkerManager {
      ...
      if (!this._client) {
        this._client = (async () => {
          this._worker = workers.createWebWorker({
            ...
            createWorker: () => new Worker(ts_worker.__worker_url_0__, { type: "module" }),
            label: this._modeId,
            keepIdleModels: true,
          ...
          if (this._defaults.getEagerModelSync()) {
            return await this._worker.withSyncedResources(
              editor.editor.getModels().filter((model) => model.getLanguageId() === this._modeId).map((model) => model.uri)
            );
          }
  ...
  class LibFiles {
    ...
    getOrCreateModel(fileName) {
      const uri = editor.Uri.parse(fileName);
      const model = editor.editor.getModel(uri);
      if (model) {
        return model;
      }
      if (this.isLibFile(uri) && this._hasFetchedLibFiles) {
        return editor.editor.createModel(this._libFiles[uri.path.slice(1)], "typescript", uri);
      }
      const matchedLibFile = src_deprecated_language_typescript_monaco_contribution_ts.typescriptDefaults.getExtraLibs()[fileName];
      if (matchedLibFile) {
        return editor.editor.createModel(matchedLibFile.content, "typescript", uri);
      }
      return null;
    }
  ...
  class DiagnosticsAdapter extends Adapter {
    constructor(_libFiles, _defaults, _selector, worker) {
        ...
        const visibleSubscription = model.onDidChangeAttached(() => {
          ...
          if (onlyVisible) {
            if (model.isAttachedToEditor()) {
              ...
              editor.editor.setModelMarkers(model, this._selector, []);
            }
          }
        });
      ...
      this._disposables.push(
        editor.editor.onDidCreateModel((model) => onModelAdd(model))
      );
      ...
      this._disposables.push({
        dispose() {
          for (const model of editor.editor.getModels()) {
      ...
      editor.editor.getModels().forEach((model) => onModelAdd(model));
    }
    dispose() {
    ...
    async _doValidate(model) {
      ...
      const relatedUris = diagnostics.map((d) => d.relatedInformation || []).reduce((p, c) => c.concat(p), []).map(
        (relatedInformation) => relatedInformation.file ? editor.Uri.parse(relatedInformation.file.fileName) : null
      );
      await this._libFiles.fetchLibFilesIfNecessary(relatedUris);
      if (model.isDisposed()) {
        return;
      }
      editor.editor.setModelMarkers(
        model,
        this._selector,
    ...
    _convertDiagnostics(model, diag) {
      ...
      if (diag.reportsUnnecessary) {
        tags.push(editor.MarkerTag.Unnecessary);
      }
      if (diag.reportsDeprecated) {
        tags.push(editor.MarkerTag.Deprecated);
      }
      return {
  ...
  class SuggestAdapter extends Adapter {
    ...
    async provideCompletionItems(model, position, _context, token) {
      ...
      const suggestions = info.entries.map((entry) => {
        ...
        if (entry.replacementSpan) {
          ...
          range = new editor.Range(p1.lineNumber, p1.column, p2.lineNumber, p2.column);
        }
        const tags = [];
        if (entry.kindModifiers !== void 0 && entry.kindModifiers.indexOf("deprecated") !== -1) {
          tags.push(editor.languages.CompletionItemTag.Deprecated);
        }
        return {
          uri: resource,
          position,
    ...
    static convertKind(kind) {
      switch (kind) {
        ...
        case Kind.keyword:
          return editor.languages.CompletionItemKind.Keyword;
        case Kind.variable:
        case Kind.localVariable:
          return editor.languages.CompletionItemKind.Variable;
        case Kind.memberVariable:
        case Kind.memberGetAccessor:
        case Kind.memberSetAccessor:
          return editor.languages.CompletionItemKind.Field;
        case Kind.function:
        case Kind.memberFunction:
        case Kind.constructSignature:
        case Kind.callSignature:
        case Kind.indexSignature:
          return editor.languages.CompletionItemKind.Function;
        case Kind.enum:
          return editor.languages.CompletionItemKind.Enum;
        case Kind.module:
          return editor.languages.CompletionItemKind.Module;
        case Kind.class:
          return editor.languages.CompletionItemKind.Class;
        case Kind.interface:
          return editor.languages.CompletionItemKind.Interface;
        case Kind.warning:
          return editor.languages.CompletionItemKind.File;
      }
      return editor.languages.CompletionItemKind.Property;
    }
...
${tagToString(tag)}`;
  ...
  class SignatureHelpAdapter extends Adapter {
    ...
    static _toSignatureHelpTriggerReason(context) {
      switch (context.triggerKind) {
        case editor.languages.SignatureHelpTriggerKind.TriggerCharacter:
        ...
        case editor.languages.SignatureHelpTriggerKind.ContentChange:
          return context.isRetrigger ? { kind: "retrigger" } : { kind: "invoked" };
        case editor.languages.SignatureHelpTriggerKind.Invoke:
        default:
          return { kind: "invoked" };
      }
    }
      ...
      return entries.flatMap((entry) => {
        return entry.highlightSpans.map((highlightSpans) => {
          return {
            ...
            kind: highlightSpans.kind === "writtenReference" ? editor.languages.DocumentHighlightKind.Write : editor.languages.DocumentHighlightKind.Text
          };
        });
      });
    }
  ...
  class DefinitionAdapter extends Adapter {
    ...
    async provideDefinition(model, position, token) {
      ...
      await this._libFiles.fetchLibFilesIfNecessary(
        entries.map((entry) => editor.Uri.parse(entry.fileName))
      );
      if (model.isDisposed()) {
        return;
      }
    ...
    async provideDocumentSymbols(model, token) {
      ...
      const convert = (item, containerLabel) => {
        const result2 = {
          name: item.text,
          detail: "",
          kind: outlineTypeTable[item.kind] || editor.languages.SymbolKind.Variable,
  ...
  outlineTypeTable[Kind.module] = editor.languages.SymbolKind.Module;
  outlineTypeTable[Kind.class] = editor.languages.SymbolKind.Class;
  outlineTypeTable[Kind.enum] = editor.languages.SymbolKind.Enum;
  outlineTypeTable[Kind.interface] = editor.languages.SymbolKind.Interface;
  outlineTypeTable[Kind.memberFunction] = editor.languages.SymbolKind.Method;
  outlineTypeTable[Kind.memberVariable] = editor.languages.SymbolKind.Property;
  outlineTypeTable[Kind.memberGetAccessor] = editor.languages.SymbolKind.Property;
  outlineTypeTable[Kind.memberSetAccessor] = editor.languages.SymbolKind.Property;
  outlineTypeTable[Kind.variable] = editor.languages.SymbolKind.Variable;
  outlineTypeTable[Kind.const] = editor.languages.SymbolKind.Variable;
  outlineTypeTable[Kind.localVariable] = editor.languages.SymbolKind.Variable;
  outlineTypeTable[Kind.variable] = editor.languages.SymbolKind.Variable;
  outlineTypeTable[Kind.function] = editor.languages.SymbolKind.Function;
  outlineTypeTable[Kind.localFunction] = editor.languages.SymbolKind.Function;
  ...
  class InlayHintsAdapter extends Adapter {
    ...
    _convertHintKind(kind) {
      switch (kind) {
        case "Parameter":
          return editor.languages.InlayHintKind.Parameter;
        case "Type":
          return editor.languages.InlayHintKind.Type;
        default:
          return editor.languages.InlayHintKind.Type;
      }
    }
  ...
  function setupMode(defaults, modeId) {
    ...
    function registerProviders() {
      ...
      if (modeConfiguration.hovers) {
        providers.push(
          editor.languages.registerHoverProvider(modeId, new QuickInfoAdapter(worker))
        );
      }
      ...
      if (modeConfiguration.rename) {
        providers.push(
          editor.languages.registerRenameProvider(
            modeId,
      ...
      if (modeConfiguration.onTypeFormattingEdits) {
        providers.push(
          editor.languages.registerOnTypeFormattingEditProvider(
            modeId,
      ...
      if (modeConfiguration.codeActions) {
        providers.push(
          editor.languages.registerCodeActionProvider(modeId, new CodeActionAdaptor(worker))
        );
      }
      if (modeConfiguration.inlayHints) {
        providers.push(
          editor.languages.registerInlayHintsProvider(modeId, new InlayHintsAdapter(worker))
        );
      }
