define("vs/mdx-C09uLMiS", ["exports", "./editor-ECyZhXSU"], (function(exports, editor) {
  "use strict";
  const conf = {
    comments: {
      blockComment: ["{/*", "*/}"]
    ...
    onEnterRules: [
      {
        beforeText: /^\s*- .+/,
        action: { indentAction: editor.languages.IndentAction.None, appendText: "- " }
      },
      {
        beforeText: /^\s*\+ .+/,
        action: { indentAction: editor.languages.IndentAction.None, appendText: "+ " }
      ...
      {
        beforeText: /<\w+/,
        action: { indentAction: editor.languages.IndentAction.Indent }
      },
      {
        beforeText: /\s+>\s*$/,
        action: { indentAction: editor.languages.IndentAction.Indent }
      ...
      {
        beforeText: /<\/\w+>/,
        action: { indentAction: editor.languages.IndentAction.Outdent }
      },
      ...Array.from({ length: 100 }, (_, index) => ({
        beforeText: new RegExp(`^${index}\\. .+`),
        action: { indentAction: editor.languages.IndentAction.None, appendText: `${index + 1}. ` }
      }))
    ]
  };
  const language = {
