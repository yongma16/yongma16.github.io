define("vs/language/html/html.worker", ["../../initialize-BJOLaCxY", "../../index-CXwHBsfm", "../../main-CLp3wP3p"], (function(initialize, index, main) {
  "use strict";
  var TokenType;
  (function(TokenType2) {
      ...
      switch (state) {
        ...
        case ScannerState.AfterOpeningEndTag:
          ...
          if (stream.skipWhitespace()) {
            return finishToken(offset, TokenType.Whitespace, index.t("Tag name must directly follow the open bracket."));
          }
          state = ScannerState.WithinEndTag;
          stream.advanceUntilChar(_RAN);
          if (offset < stream.pos()) {
            return finishToken(offset, TokenType.Unknown, index.t("End tag name expected."));
          }
          return internalScan();
        case ScannerState.WithinEndTag:
          ...
          if (emitPseudoCloseTags && stream.peekChar() === _LAN) {
            state = ScannerState.WithinContent;
            return finishToken(offset, TokenType.EndTagClose, index.t("Closing bracket missing."));
          }
          errorMessage = index.t("Closing bracket expected.");
          break;
        case ScannerState.AfterOpeningStartTag:
          ...
          stream.advanceUntilChar(_RAN);
          if (offset < stream.pos()) {
            return finishToken(offset, TokenType.Unknown, index.t("Start tag name expected."));
          }
          return internalScan();
        case ScannerState.WithinTag:
          ...
          if (emitPseudoCloseTags && stream.peekChar() === _LAN) {
            state = ScannerState.WithinContent;
            return finishToken(offset, TokenType.StartTagClose, index.t("Closing bracket missing."));
          }
          stream.advance(1);
          return finishToken(offset, TokenType.Unknown, index.t("Unexpected character in tag."));
  ...
  function isLetterOrDigit(text, index2) {
    const c = text.charCodeAt(index2);
...
`;
  ...
  class HTMLCompletion {
    ...
    _doComplete(document, position, htmlDocument, settings) {
      ...
      function collectCharacterEntityProposals() {
        ...
        if (k >= 0 && text[k] === "&") {
          const range = main.Range.create(main.Position.create(position.line, characterStart - 1), position);
          for (const entity in entities) {
            if (endsWith(entity, ";")) {
              ...
              result.items.push({
                ...
                documentation: index.t("Character entity representing '{0}'", entities[entity]),
      ...
      function getEntityHover(text2, range) {
        ...
        for (const entity in entities) {
          ...
          if (currEntity === label) {
            ...
            hex += code;
            const contentsDoc = index.t("Character entity representing '{0}', unicode equivalent '{1}'", entities[entity], hex);
            if (contentsDoc) {
              hover = { contents: contentsDoc, range };
            } else {
              hover = null;
  ...
  function validateAndCleanURI(uriStr, document) {
    try {
      let uri = index.URI.parse(uriStr);
  ...
  function provideFileSymbolsInternal(document, node, symbols) {
    ...
    const symbol = main.DocumentSymbol.create(name, void 0, main.SymbolKind.Field, range, range);
    symbols.push(symbol);
  ...
  class HTMLFolding {
    ...
    limitRanges(ranges, rangeLimit) {
      ...
      const setNestingLevel = (index2, level) => {
        nestingLevels[index2] = level;
        if (level < 30) {
            ...
            if (m) {
              if (m[1]) {
                ...
                if (i >= 0) {
                  ...
                  if (endLine > startLine && prevStart !== startLine) {
                    addRange({ startLine, endLine, kind: main.FoldingRangeKind.Region });
                  }
                }
              }
            } else {
  ...
  class HTMLSelectionRange {
    ...
    getSelectionRange(position, document, htmlDocument) {
      ...
      let prev = void 0;
      let current = void 0;
      for (let index2 = applicableRanges.length - 1; index2 >= 0; index2--) {
        const range = applicableRanges[index2];
        if (!prev || range[0] !== prev[0] || range[1] !== prev[1]) {
          current = main.SelectionRange.create(main.Range.create(document.positionAt(applicableRanges[index2][0]), document.positionAt(applicableRanges[index2][1])), current);
        }
        prev = range;
      }
      if (!current) {
    ...
    getApplicableRanges(document, position, htmlDoc) {
      ...
      if (currNode.startTagEnd && !currNode.endTagStart) {
        ...
        const closeRange = main.Range.create(document.positionAt(currNode.startTagEnd - 2), document.positionAt(currNode.startTagEnd));
    ...
    getAttributeLevelRanges(document, currNode, currOffset) {
      const currNodeRange = main.Range.create(document.positionAt(currNode.start), document.positionAt(currNode.end));
