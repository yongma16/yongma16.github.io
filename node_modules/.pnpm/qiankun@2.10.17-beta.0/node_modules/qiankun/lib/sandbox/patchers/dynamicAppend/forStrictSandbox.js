 ...
 * @since 2020-10-13
 */
var elementAttachedSymbol = Symbol('attachedApp'); // Get native global window with a sandbox disgusted way, thus we could share it between qiankun instances🤪
...
var rawHeadInsertBefore = HTMLHeadElement.prototype.insertBefore; // Share proxyAttachContainerConfigMap between multiple qiankun instance, thus they could access the same record
  ...
  if (speedy) {
    ...
    var proxyDocument = new Proxy(document, {
      ...
      get: function get(target, p, receiver) {
          ...
          case 'createElement':
            {
              ...
              return function createElement() {
                ...
                var element = targetCreateElement.call.apply(targetCreateElement, [target].concat(args)); // only record the element which is created by the current sandbox, thus we can avoid the element created by nested sandboxes
        ...
        var value = target[p]; // must rebind the function to the target otherwise it will cause illegal invocation error
    ...
    sandbox.patchDocument(proxyDocument); // patch MutationObserver.prototype.observe to avoid type error
...
function patchStrictSandbox(appName, appWrapperGetter, sandbox) {
  ...
  return function free() {
    ...
    if (mounting) (0, _common2.calcAppCount)(appName, 'decrease', 'mounting'); // release the overwritten prototype after all the micro apps unmounted
